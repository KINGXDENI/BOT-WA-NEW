...
if (pairingCode && !NanoBotz.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +6282144323683")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Silakan Masukan Nomer WhatsApp Bot nya 🐦\nFor example: +6282144323683 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
...
try{
		...
		console.log(chalk.white(chalk.bgBlack(`⏳ Menghubungkan...`)))
		}
...
console.log('succses terkirim');
      ...
      app.listen(port, () => {
       ...
       NanoBotz.sendMessage('120363297408931463@g.us', {
         text: `*Bot started!*`
       });
     console.log(chalk.black(chalk.bgGreen.bold(`✅ Berhasil Terhubung...`)));
    
     console.log(chalk.white.bold('🌿 Connected to:'));
     console.log(chalk.cyanBright('===================='));
     console.log(chalk.green(`📲 ID:        ${NanoBotz.user.id}`));
     console.log(chalk.green(`🆔 LID:       ${NanoBotz.user.lid}`));
     console.log(chalk.green(`👤 Name:      ${NanoBotz.user.name || "No Name"}`));
     console.log(chalk.cyanBright('===================='));
...
welcome(iswel, isLeft, NanoBotz, anu)
    ...
    async function getMessage(key){
        ...
        return {
            conversation: "Dibo Bot Ada Di Sini"
        }
    }
