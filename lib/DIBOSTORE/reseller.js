...
const baseURL = 'https://apistore.dibo.my.id';
...
function sendNomorToPulsa(nomorKontak) {
    return new Promise((resolve, reject) => {
        const url = `${baseURL}/pulsa`; // Endpoint for sending nomor
        const bodys = { nomor: nomorKontak }; // Prepare the body with the nomorKontak
...
function getUserByNomor(nomorKontak) {
    return new Promise((resolve, reject) => {
        const url = `${baseURL}/users/nomor/${nomorKontak}`; // Endpoint for getting user by nomor
    ...
    return new Promise((resolve, reject) => {
        ...
        getUserByNomor(nomorReseller)
            .then(user => {
                ...
                const url = `${baseURL}/pulsa/transaksi`; // Endpoint for performing the transaction
...
function getProdukPLN() {
    return new Promise((resolve, reject) => {
        const url = `${baseURL}/produk/kategori/3`; // Endpoint untuk produk PLN
        ...
        fetch(url, {
            ...
            .then(data => {
                resolve(data); // Resolving data jika request berhasil
            })
            .catch(error => {
                reject(`Error fetching PLN products: ${error.message}`); // Reject dengan pesan error
            });
    });
}
module.exports = {
